...
Start (modern Compose V2 with deploy.resources.reservations.devices support)
```bash
...
Compatibility notes
- Use the **legacy file** (`docker-compose-ollama-with-open-webui.legacy.yml`) if your `docker compose` rejects `deploy.resources.reservations.devices` (older Compose versions).
- Use the **primary file** (`docker-compose-ollama-with-open-webui.yml`) if you have Docker Compose V2 with full `deploy.resources.reservations.devices` support.
